...
console.log("DEBUG PRICE", exec.symbol, marketData[exec.symbol], exec.entryPrice);
exec.currentPrice = marketData[exec.symbol];      if (exec.entryPrice) {
        exec.actualReturnPct = ((exec.currentPrice - exec.entryPrice) / exec.entryPrice) * 100;
        const daysHeld = (Date.now() - exec.timestamp) / (1000 * 60 * 60 * 24);
        const monthsHeld = daysHeld / 30;
        const expectedReturn = (Math.pow(1 + exec.expectedMonthlyDrift / 100, monthsHeld) - 1) * 100;
        exec.driftStatus = monthsHeld < 0.5 ? "TOO_EARLY" : exec.actualReturnPct >= expectedReturn * 0.95 ? "BEATING" : exec.actualReturnPct >= expectedReturn * 0.80 ? "ON_TRACK" : "LAGGING";
      }
      exec.lastUpdated = Date.now();
    }
  });
